    ...
    const generateAIResponse = (userMessage) => {
        ...
        if (msg.match(/task|todo|what.*do|pending|remaining/i)) {
            ...
            if (inProgressTasks.length > 0) {
                ...
                inProgressTasks.slice(0, 3).forEach(t => { response += `• ${t.title}\n`; });
                if (inProgressTasks.length > 3) response += `  ...and ${inProgressTasks.length - 3} more\n`;
                response += `\n`;
            }
            if (todoTasks.length > 0) {
                response += `**To Do:**\n`;
                todoTasks.slice(0, 3).forEach(t => { response += `• ${t.title}\n`; });
                    ...
                    <div className="px-2 py-3 border-b border-slate-700/50 flex items-center gap-2 mb-3">
                        ...
                        <div className="flex -space-x-2">
                            {teamMembers?.slice(0, 6).map(member => (
                                <Avatar key={member.id || member} name={member.name || member} size="xs" className="border-2 border-slate-900" />
                            ))}
...
const AIAgentPanel = () => {
    ...
    return (
        <>
            {/* Backdrop */}
            <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50" onClick={closePanel} />
            ...
            <div className="fixed right-0 top-0 h-full w-full max-w-lg bg-slate-900 border-l border-slate-700 shadow-2xl z-50 flex flex-col animate-slide-in-right">
                ...
                <div className="flex-1 overflow-y-auto p-4">
                    ...
                    {activeTab === 'planner' && (
                        <div className="space-y-4">
                            <div>
                                <label className="block text-sm font-medium text-slate-300 mb-2">Select a project</label>
                                <select
                                    value={selectedProject}
                                    onChange={(e) => setSelectedProject(e.target.value)}
                            ...
                            {selectedProject && suggestions.length > 0 && (
                                <div className="space-y-3">
                                    <p className="text-sm text-slate-400">Based on your project, here are suggested tasks:</p>
                            ...
                            {selectedProject && suggestions.length === 0 && (
                                <div className="text-center py-8">
                                    ...
                                    <p className="text-slate-400 text-sm">This project already has comprehensive task coverage.</p>
                                </div>
                            )}
