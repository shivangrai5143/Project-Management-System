    ...
    return (
        <div className="space-y-4">
            ...
            <div className="flex flex-wrap items-center gap-4 text-xs text-slate-400">
                <div className="flex items-center gap-2"><div className="w-3 h-3 rounded bg-indigo-500/40" /><span>Scheduled</span></div>
                <div className="flex items-center gap-2"><div className="w-3 h-3 rounded bg-orange-500/40" /><span>High Priority</span></div>
                <div className="flex items-center gap-2"><div className="w-3 h-3 rounded bg-red-500/40" /><span>Overdue</span></div>
                <div className="flex items-center gap-2"><div className="w-3 h-3 rounded bg-emerald-500/40" /><span>Completed</span></div>
            </div>
        </div>
    );
};

const ProjectDetailPage = ({ projectId: propProjectId }) => {
        ...
        <div className="space-y-6 animate-fade-in">
            ...
            <div className="flex items-start justify-between gap-4">
                ...
                >
                    {(close) => (
                        <>
                            <DropdownItem icon={Edit} onClick={() => { setIsEditModalOpen(true); close(); }}>
                                Edit Project
                            </DropdownItem>
                            <DropdownItem icon={UserPlus}>Add Members</DropdownItem>
                            <DropdownItem icon={Settings}>Settings</DropdownItem>
                            <DropdownDivider />
                            <DropdownItem icon={Trash2} danger onClick={() => { handleDeleteProject(); close(); }}>
                                Delete Project
                            </DropdownItem>
                        </>
                    )}
                ...
                <div className="flex items-center gap-2">
                    ...
                    <div className="flex -space-x-2">
                        {projectMembers.slice(0, 5).map(member => (
                            <Avatar key={member.id} name={member.name} size="sm" className="border-2 border-slate-800" />
                        ))}
            ...
            {activeTab === 'list' && <ListView tasks={tasks} team={team} />}
            ...
            <Modal isOpen={isEditModalOpen} onClose={() => setIsEditModalOpen(false)} title="Edit Project">
                <ProjectForm project={project} onSubmit={handleUpdateProject} onCancel={() => setIsEditModalOpen(false)} />
            </Modal>
